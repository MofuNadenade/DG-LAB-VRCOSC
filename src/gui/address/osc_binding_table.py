...
class OSCBindingTableTab(QWidget):
    ...
    def refresh_binding_table(self) -> None:
        ...
        for row, (addr, action) in enumerate(bindings):
            self.update_binding(row, addr, action)
        ...
        for row, (addr, action) in enumerate(bindings):
            # 验证绑定有效性
            is_valid, _ = self.validate_binding(addr, action)
            if is_valid:
                valid_count += 1
            else:
                invalid_count += 1
    ...
    def add_binding(self) -> None:
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            if dialog.validate_input():
                ...
                # 添加绑定
                try:
                    # TODO 直接添加到binding_table，不要注册到registries
    ...
    def delete_binding(self) -> None:
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                # TODO 直接从binding_table删除，不要注册到registries
