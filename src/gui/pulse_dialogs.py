...
class NewPulseDialog(QDialog):
    ...
    def validate_input(self) -> bool:
        ...
        name = self.name_edit.text().strip()
        if not name:
            QMessageBox.warning(self, translate("pulse_dialogs.new_pulse.input_error"), translate("pulse_dialogs.new_pulse.name_required"))
            return False
        return True
...
class ImportPulseDialog(QDialog):
    ...
    def load_pulses(self, file_path: str) -> None:
        ...
        try:
            ...
            QMessageBox.critical(self, translate("pulse_dialogs.import_pulse.file_error"), translate("pulse_dialogs.import_pulse.file_not_found"))
            return
        except json.JSONDecodeError as e:
            QMessageBox.critical(self, translate("pulse_dialogs.import_pulse.format_error"), translate("pulse_dialogs.import_pulse.json_invalid").format(str(e)))
            return
        except UnicodeDecodeError:
            QMessageBox.critical(self, translate("pulse_dialogs.import_pulse.encoding_error"), translate("pulse_dialogs.import_pulse.encoding_unsupported"))
            return
        except Exception as e:
            QMessageBox.critical(self, translate("pulse_dialogs.import_pulse.read_error"), translate("pulse_dialogs.import_pulse.read_failed").format(str(e)))
            return
        ...
        try:
            ...
            for name, pulse_data in pulses.items():
                ...
                if is_valid:
                    ...
                    if integrity_warnings:
                        item.setToolTip(f"{translate('pulse_dialogs.import_pulse.preview.warnings')}: {'; '.join(integrity_warnings)}")
            ...
            if valid_count > 0:
                ...
                if error_count > 0:
                    QMessageBox.critical(self, translate("pulse_dialogs.import_pulse.import_failed"), translate("pulse_dialogs.import_pulse.all_invalid"))
                else:
                    QMessageBox.warning(self, translate("pulse_dialogs.import_pulse.import_failed"), translate("pulse_dialogs.import_pulse.no_valid_data"))
                    
        except KeyError as e:
            QMessageBox.critical(self, translate("pulse_dialogs.import_pulse.data_error"), translate("pulse_dialogs.import_pulse.missing_field").format(str(e)))
        except Exception as e:
            QMessageBox.critical(self, translate("pulse_dialogs.import_pulse.process_error"), translate("pulse_dialogs.import_pulse.process_failed").format(str(e)))
...
class ExportPulseDialog(QDialog):
    ...
    def export_pulses(self) -> None:
        ...
        if not file_path:
            QMessageBox.warning(self, translate("pulse_dialogs.export_pulse.path_error"), translate("pulse_dialogs.export_pulse.select_save_path"))
            return
            
        selected_items = self.pulse_list.selectedItems()
        if not selected_items:
            QMessageBox.warning(self, translate("pulse_dialogs.export_pulse.selection_error"), translate("pulse_dialogs.export_pulse.select_at_least_one"))
            return
        ...
        try:
            ...
            QMessageBox.critical(self, translate("pulse_dialogs.export_pulse.permission_error"), translate("pulse_dialogs.export_pulse.no_write_permission"))
        except OSError as e:
            QMessageBox.critical(self, translate("pulse_dialogs.export_pulse.file_error"), translate("pulse_dialogs.export_pulse.cannot_create").format(str(e)))
        except Exception as e:
            QMessageBox.critical(self, translate("pulse_dialogs.export_pulse.export_error"), translate("pulse_dialogs.export_pulse.save_failed").format(str(e)))
...
class PulseInfoDialog(QDialog):
    ...
    def setup_ui(self) -> None:
        ...
        if intensities:
            form_layout.addRow(translate("pulse_dialogs.pulse_info.min_intensity_label"), QLabel(f"{min(intensities)}%"))
            form_layout.addRow(translate("pulse_dialogs.pulse_info.max_intensity_label"), QLabel(f"{max(intensities)}%"))
            form_layout.addRow(translate("pulse_dialogs.pulse_info.avg_intensity_label"), QLabel(f"{sum(intensities)/len(intensities):.1f}%"))
