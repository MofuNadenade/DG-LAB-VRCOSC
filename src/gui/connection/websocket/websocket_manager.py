...
from config import validate_ip, save_settings
...
class WebSocketConnectionSignals(QObject):
    """WebSocket连接组件的信号类"""
...
class WebSocketConnectionManager:
    ...
    def __init__(self, ui_interface: UIInterface):
        ...
        # 信号组件 - 使用组合模式
        self.signals: WebSocketConnectionSignals = WebSocketConnectionSignals()
    ...
    def save_settings(self, interface_name: str, ip: str, websocket_port: int,
                      enable_remote: bool, remote_address: str) -> None:
        """保存WebSocket设置"""
        if 'connection' not in self.settings:
            self.settings['connection'] = {}

        if 'websocket' not in self.settings['connection']:
            self.settings['connection']['websocket'] = {}

        self.settings['connection']['websocket']['interface'] = interface_name
        self.settings['connection']['websocket']['ip'] = ip
        self.settings['connection']['websocket']['port'] = websocket_port
        self.settings['connection']['websocket']['enable_remote'] = enable_remote
        self.settings['connection']['websocket']['remote_address'] = remote_address

        save_settings(self.settings)
        logger.info("WebSocket设置已保存")
    ...
    def start_connection(self, selected_ip: str, websocket_port: int, osc_port: int,
        ...
        logger.info(f"正在启动WebSocket连接: {selected_ip}:{websocket_port}, OSC端口: {osc_port}")
    ...
    async def _run_server(self, ip: str, websocket_port: int, osc_port: int,
        ...
        try:
            ...
            logger.error(f"WebSocket连接运行异常: {str(e)}")
