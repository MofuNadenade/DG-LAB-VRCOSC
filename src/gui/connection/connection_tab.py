...
from PySide6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QGroupBox, QFormLayout,
                               QComboBox, QSpinBox, QLabel, QPushButton, QCheckBox, QMessageBox, QStackedWidget)
...
class ConnectionTab(QWidget):
    def __init__(self, ui_interface: UIInterface) -> None:
        ...
        self.connection_mode_checkbox: QCheckBox
    ...
    def init_ui(self) -> None:
        ...
        # 连接模式切换复选框（WebSocket/蓝牙）
        self.connection_mode_checkbox = QCheckBox(translate("connection_tab.use_bluetooth"))
        self.connection_mode_checkbox.stateChanged.connect(self.on_connection_mode_changed)
        self.connection_mode_label = QLabel(translate("connection_tab.connection_mode_label"))
        self.connection_mode_layout.addRow(self.connection_mode_label, self.connection_mode_checkbox)
    ...
    def load_settings(self) -> None:
        ...
        if connection_mode == ConnectionMode.BLUETOOTH.value:
            ...
            self.connection_mode_checkbox.setChecked(True)
            self.connection_stack.setCurrentWidget(self.bluetooth_widget)
        else:
            self.current_connection_mode = ConnectionMode.WEBSOCKET
            self.connection_mode_checkbox.setChecked(False)
            self.connection_stack.setCurrentWidget(self.websocket_widget)
        
        self.connection_mode_checkbox.setEnabled(True)

    def on_connection_mode_changed(self, state: int) -> None:
        """连接模式切换处理"""
        if state:
            # 切换到蓝牙模式
            self.current_connection_mode = ConnectionMode.BLUETOOTH
    ...
    def update_connection_state(self, state: ConnectionState, message: str = "") -> None:
        ...
        # 更新连接模式复选框的启用状态
        self.connection_mode_checkbox.setEnabled(state == ConnectionState.DISCONNECTED)
    ...
    def update_ui_texts(self) -> None:
        ...
        self.connection_mode_checkbox.setText(translate("connection_tab.use_bluetooth"))
