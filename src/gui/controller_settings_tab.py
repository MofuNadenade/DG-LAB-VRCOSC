...
class ControllerSettingsTab(QWidget):
    ...
    def init_ui(self) -> None:
        ...
        # 控制器参数设置
        controller_group = QGroupBox(_("controller_tab.title"))
        self.controller_group = controller_group  # 保持引用以便启用/禁用
        controller_group.setEnabled(False)  # 默认禁用
        controller_form = QFormLayout()
        
        # 添加 A 通道滑动条和标签
        self.a_channel_label = QLabel(f"A {_('controller_tab.channel_intensity')}: 0 / 100")
        ...
        # 添加 B 通道滑动条和标签
        self.b_channel_label = QLabel(f"B {_('controller_tab.channel_intensity')}: 0 / 100")
        ...
        # 功能选项
        self.fire_mode_disabled_checkbox = QCheckBox(_("controller_tab.disable_fire_mode"))
        self.fire_mode_disabled_checkbox.setChecked(False)
        controller_form.addRow(self.fire_mode_disabled_checkbox)
        
        self.enable_panel_control_checkbox = QCheckBox(_("controller_tab.enable_panel_control"))
        ...
        self.dynamic_bone_mode_a_checkbox = QCheckBox(f"A{_('controller_tab.interaction_mode')}")
        self.dynamic_bone_mode_b_checkbox = QCheckBox(f"B{_('controller_tab.interaction_mode')}")
        self.current_channel_label = QLabel(f"{_('controller_tab.current_panel_channel')}: {_('controller_tab.not_set')}")
    ...
    def update_ui_texts(self) -> None:
        ...
        self.enable_panel_control_checkbox.setText(_("controller_tab.enable_panel_control"))
        self.enable_chatbox_status_checkbox.setText(_("controller_tab.enable_chatbox"))
        self.dynamic_bone_mode_a_checkbox.setText(f"A{_('controller_tab.interaction_mode')}")
        self.dynamic_bone_mode_b_checkbox.setText(f"B{_('controller_tab.interaction_mode')}")
        self.save_settings_btn.setText(_("osc_address_tab.save_config"))

        # 更新通道强度标签
        a_value = self.a_channel_slider.value()
        b_value = self.b_channel_slider.value()
        self.a_channel_label.setText(f"A {_('controller_tab.channel_intensity')}: {a_value} / 100")
        self.b_channel_label.setText(f"B {_('controller_tab.channel_intensity')}: {b_value} / 100")
        ...
        if ": " in current_channel_text:
            ...
            if current_channel in [_('controller_tab.not_set'), "not_set", "未设置", "未設定"]:
                self.current_channel_label.setText(f"{_('controller_tab.current_panel_channel')}: {_('controller_tab.not_set')}")
            else:
                self.current_channel_label.setText(f"{_('controller_tab.current_panel_channel')}: {current_channel}")
        else:
            self.current_channel_label.setText(f"{_('controller_tab.current_panel_channel')}: {_('controller_tab.not_set')}")
