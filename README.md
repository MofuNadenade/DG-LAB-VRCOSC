...
- **官方波形导入**: 支持导入DG-LAB官方.pulse波形文件，完全兼容官方格式

### 📹 录制回放功能
- **实时录制**: 支持实时录制设备输出波形和强度变化
- **会话管理**: 完整的录制会话管理，支持暂停、继续、停止
- **文件管理**: 支持录制文件的保存、导入、导出和删除
- **回放控制**: 支持录制内容的播放、暂停、循环播放
- **格式支持**: 使用DGR格式保存录制数据，支持完整波形信息
- **多语言**: 完整的中文、英文、日文界面支持
...
#### 录制功能
1. 确保设备已连接（蓝牙或WebSocket）
2. 切换到"录制回放"标签页
3. 点击"开始录制"按钮开始录制设备输出
4. 录制过程中可以暂停、继续或停止录制
5. 停止录制后，录制内容会自动保存到文件列表
